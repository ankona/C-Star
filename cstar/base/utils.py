...
def _update_user_dotenv(key_value_pairs: dict[str, str]) -> None:
    env_file = Path(CSTAR_USER_ENV_PATH).expanduser()
    log.info(f"Updating environment in C-Star configuration file {env_file}")

    key_value_strings = (f"{k}={v}" for k, v in key_value_pairs.items())
    ...
    if not env_file.exists():
        ...
        env_file_str = f"{base_env_str}{env_body}\n"
    else:
        env_file_str = f"\n{env_body}\n"
