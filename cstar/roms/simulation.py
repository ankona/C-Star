...
class ROMSSimulation(Simulation):
    ...
    def _check_forcing_collection_types(
        ...
        if not all(isinstance(ind, expected_class) for ind in collection):
            cls_name = expected_class.__name__
            msg = f"ROMSSimulation.{collection} must be a list of {cls_name} instances"
            raise TypeError(msg)
    ...
    def from_dict(
        ...
        if discretization_kwargs is not None:
            discretization = Discretization(**discretization_kwargs)
